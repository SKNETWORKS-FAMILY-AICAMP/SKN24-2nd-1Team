...
1. 과적합(Overfitting) 방지 시도  
    •    L1 규제: 파라미터 값을 10까지 조정하며 실행  
    •    L2 규제: 파라미터 값을 50까지 조정하며 실행  
    •    Cross-Validation: 일반 교차 검증 적용  
    •    GroupKFold: 데이터 특성을 고려한 그룹별 교차 검증 실시  
    •    Feature Importance: 변수 중요도 분석을 통해 불필요한 변수 제거 후 재학습  
